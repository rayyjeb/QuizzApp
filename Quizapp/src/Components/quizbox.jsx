...
import Questiondata from "../resources/questionData";
import { AppContext } from "./context";
//Function to make Question Box
function QuestionBox({ resultPage, i }) {
  const [darktheme, setDarkMode] = useState(true);
  const [highlightfn, sethighlightfn] = useState(true);
  const togglemode = () => {
  ...
  return (
    ...
    <div
      style={{
        backgroundColor: darktheme ? "#25243C" : "#FFFFFF",
        height: "100vh",
        margin: "0",
        padding: "0",
      }}
    >
      ...
      <div className="navBarParent">
        <nav>
          <h1
            className="logo"
            style={{
              color: darktheme ? "#FFFFFF" : "#25243C",
            }}
          >
            QUIZZR.
          </h1>
          {/* TOGGLING THEME BUTTON */}
          <button
            className="Modes"
            onClick={togglemode}
            style={{
              backgroundColor: darktheme ? "#302F4D" : "#282828",
            }}
          >
            {darktheme ? (
              <svg
      ...
      {/* BODY ELEMENTS */}
      <div className="QnsDiv">
        {/* CIRCLE WHERE THE NTH NUMBER OF QUESTION IS DISPLAYED */}
        <div
          className="noOfQns"
          style={{
            backgroundColor: darktheme ? "#302F4D" : "#282828",
            borderColor: darktheme ? "#68AF56" : "#FFE500",
          }}
        >
          {i + 1}
        </div>
      ...
      <div className="mainQuizBox">
        ...
        >
          ...
          <div className="questionDiv">
            <div className="ParentQuestionDiv">
              <h1
                className="QuestionHeader"
                style={{
                  color: highlightfn ? "white" : "yellow",
                }}
              >
            ...
            <div className="options">
              <button
                onClick={() => {
                  resultPage(i, Questiondata[i].optionA);
                }}
                className="optionBtn"
                style={{
                  backgroundColor: darktheme ? "#25243C    " : "#474747",
                }}
              >
                {Questiondata[i].optionA}
              </button>
              <button
                onClick={() => {
                  resultPage(i, Questiondata[i].optionB);
                }}
                className="optionBtn"
                style={{
                  backgroundColor: darktheme ? "#25243C" : "#474747",
              ...
              >
                {Questiondata[i].optionD}
              </button>
            </div>
            <div className="parentHighlight ">
              <div className="highlightBtn">
                <button
                  className="actionBtns"
                  // FUNCTION TO MAKE HIGHLIGHT FEATURE
                  onClick={() => sethighlightfn(false)}
                  style={{
                    backgroundColor: darktheme ? "#9CE38B" : "#FFE500",
                  }}
                >
                  HIGHLIGHT
                </button>
